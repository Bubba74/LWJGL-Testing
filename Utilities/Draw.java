...
import Main.Fighter;
import Main.Firework;
import Main.Obstacle;
import Main.Powerup;
import Main.Projectile;
import Main.Powerup.type;
...
//		GL11.glDisable(GL_BLEND);
	...
	public static void drawCircle(int x, int y, int radius, String style){
		GL11.glPushMatrix();
		GL11.glLoadIdentity();
		...
		if (style.equals("fill")){
			...
			for (int i = -180;i<180;i++){
				...
				glVertex2f((float)Functions.circleValue(i, true)*radius*2,(float)Functions.circleValue(i, false)*radius*2);
			}
			...
			for (int i = -180;i<180;i++){
				...
				glVertex2f((float)Functions.circleValue(i, true)*radius,(float)Functions.circleValue(i, false)*radius);
			}
	...
	public static void drawProj(Projectile p){
		float newX = (float) (p.getX()+p.getDeltaX()*p.getSpeed());
		float newY = (float)(p.getY()+p.getDeltaY()*p.getSpeed());
		p.setX(newX);
		p.setY(newY);
